        ...
        saveFormData() {
            this.$refs['formData'].validate(valid => {
                ...
                if (this.formData.id || this.formData.id == 0) {//修改一条教师数据
                    ...
                    console.log("提交对象1", this.formData)
                } else {//新增一条教师数据

                    if (this.formRules.phone != null && this.formRules.name != null) {

                        if (this.formData.name == "" && this.formData.phone == "") {
                            alert("警告！姓名 and 电话不能为空！");
                            return callback();
                            ...
                            console.log("提交对象2", this.formData)
                            //return callback();
                        }
                    }else{
                        axios.post('/api/score/scTeacher/insert', this.formData).then((resp) => {
                ...
                //数据校验
                // if (this.formRules.name == null) {
                //     return callback(new Error('姓名不能为空'));
                // } else if (this.formRules.phone == null) {
